...
536870912 , 603979776, 805306368, 905969664/)
    ...
    do ith = H%ith_start(H%MpiId), H%ith_end(H%MpiId)   ! 0 <= cos theta < 1
        ...
        lam_mm = sq4pi_inv ! lamda_00 1/sqrt(4pi)
        scalem=1
        a_ix = 0
        a_w = -1._dp / sth

        do m = 0, mmax_ring ! this whole part transform alm to Q and U
            fm  = DBLE(m)
            f2m = 2.0_dp * fm
            fm2 = fm * fm
            fm_on_s2 = fm * one_on_s2 ! on_s2 = /sin^2
            ...
            if (abs(lam_mm) < UNFLOW) then ! unflow ~ 1e-30, ovflow ~1 e 30
            ...
            do l = m+1, nlmax-1, 2
                ...
                lam_lm = lam_2 * corfac !corfac= actual lam_mm actual lambda_lm, OVFLOW factors removed
            ...
            enddo  ! lam_lm is actual lambda_lm,
        ...
        call spinring_synthesis(H,nlmax, b_north_U, nph, ringI, kphi0,mmax_ring)!b_m/s_Q/U is input, ringR/I is output, ringR/I is real/imaginary part of grad phi
