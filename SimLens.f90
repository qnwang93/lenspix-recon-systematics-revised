    ...
    Type(HealpixPower) :: P, Phiiii
    Type(HealpixAlm)   :: A, GPhi
    ...
    if (H%MpiID ==0) then !if we are main thread
        ...
        call HealpixAlm_nullify(GPhi)
        call HealpixPower_nullify(Phiiii)
        ...
        call HealpixAlm2GradientMap(H,A, GradPhi,npix,'PHI') ! If want to calculate grad of T(seemingly. or E, B?) need to change 'PHI' to sth starts with T
	...
	GradPhi%TQU = M%TQU
	GradPhi%npix = M%npix
!	GradPhi%nmaps = M%nmaps
!	GradPhi%ordering = M%ordering
	GradPhi%nside = M%nside
!	GradPhi%spin = 1
	...
	call HealpixAlm2Power(A,P)
        ...
        call HealpixMap2Power(H, GradPhi, Phiiii, lmax)
        call HealpixMap_Write(M, 'lensed_map.fits')
        call HealpixPower_Write(Phiiii, 'phiiii.txt')

	write (*,*) M%nmaps, M%HasPhi, M%spin, '\n'
        write (*,*) GradPhi%nmaps, GradPhi%HasPhi, GradPhi%spin, '\n'
	write (*,*) P%pol, P%lens, '\n'
	write (*,*) Phiiii%pol, Phiiii%lens, '\n'
	write (*,*) GradPhi%SpinField(10)

        !Save map to .fits file
    end if
	
!	call HealpixFree(Phiiii)
!	call HealpixFree(GradPhi)
#ifdef MPIPIX
    call HealpixFree(H)
    call mpi_finalize(i)
#endif
